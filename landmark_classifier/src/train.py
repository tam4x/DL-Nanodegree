...
def train_one_epoch(train_dataloader, model, optimizer, loss):
    ...
    for batch_idx, (data, target) in tqdm(
        ...
        train_acc += torch.sum(torch.squeeze(pred.eq(target.data.view_as(pred))).cpu())
        total += data.size(0)
...
def valid_one_epoch(valid_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            pred = torch.argmax(output, dim = 1)
            valid_acc += torch.sum(torch.squeeze(pred.eq(target.data.view_as(pred))).cpu())
            total += data.size(0)
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor= 0.5, min_lr = 0.05)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        print('\nTrain Accuracy: %2d%%, Valid Accuracy: %2d%% ' % (train_acc, valid_acc))
...
def test_train_one_epoch(data_loaders, optim_objects):
    ...
    for _ in range(2):
        lt, vl = train_one_epoch(data_loaders['train'], model, optimizer, loss)
...
def test_valid_one_epoch(data_loaders, optim_objects):
    ...
    for _ in range(2):
        lv, vl = valid_one_epoch(data_loaders["valid"], model, loss)
